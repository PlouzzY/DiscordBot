...
async def on_member_join(member): # функция срабатывает, если пользователь присоединился к серверу
    ...
    await chnlJoin.send(f"✅ {member} зашел на сервер!") # отправляем сообщение (можно сделать через эмбед)

@bot.event # создаем ивент
async def on_member_remove(member): # функция срабатывает, если пользователь покинул сервер
    chnlExit = bot.get_channel(1225114215170310207) # получаем канал, в который будут скидываться сообщения о заходе
    await chnlExit.send(f"❌ {member} покинул сервер!") # отправляем сообщение (можно сделать через эмбед)
